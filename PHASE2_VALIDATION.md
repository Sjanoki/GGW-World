...
- **Full-screen Nav + COMMS console** – The NavStation modal now expands to nearly the entire viewport, showing the CRT radar, orbit trace, scaled stats, and a COMMS log/control panel with canned messages that append to a local log and echo to stdout.
- **Doors and devices** – Interacting with doors, beds, dispensers, and other fixtures still works, door toggles update the tile map (with a visual indicator), and the food generator feeds the pawn directly without a modal.
...
- **Config-driven tanks & power draws** – Default tank contents (pure O₂) and each device’s idle/online power draw/output come from `config/game_config.toml`, keeping the ship’s cabin air and energy budget tunable without recompiles.
- **Ship Computer power overview** – The Ship Computer modal renders the authoritative power summary (generation/load/net) along with grouped device rows that can be toggled on/off for non-critical systems directly from the console.
...
- Enforce power gating so offline devices actually lose their functionality (right now toggles update the summary/HUD only).
